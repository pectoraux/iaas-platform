...
WORK-001 is VERIFIED. WORK-001 through WORK-019 are VERIFIED in dependency order. `IAAS-DOM-ARCH-4` is the current frozen architecture until WORK-020 freezes V5. WORK-020 is READY and is the only eligible Work Item.
...
Objective: establish persistent governance/specification without changing production behavior.
Repository Scope: `spec/` governance documents and executable consistency gate.
Architecture Constraints: frozen governance architecture; no production implementation; changes require ACR.
...
Required Verification: repository specification inspection; automated consistency validator; negative tests; CI; diff-scope inspection; independent Architect Review.
Definition of Done: specification committed; gates pass; no production diff; architect approves; PR merged; VERIFIED.
...
Architecture Constraints: preserve durable evidence and kernel/Data Plane/Economic Pipeline boundaries.
...
Objective: make affected PostgreSQL fixtures explicit and deterministic.
...
Objective: reconcile verified repository reality into domain planning.
...
Architecture Constraints: tenant-scoped catalog; no execution; PostgreSQL durable; no kernel/vertical/economic/data-plane coupling.
...
Objective: reconcile V3 with verified Transform Stack reality.
...
Required Verification: validator; truth-regression tests; architecture/Typecheck/PG/lint/scope; Architect Review.
...
Objective: prove Registry → Runtime → Record is one coherent subsystem.
Repository Scope: conformance tests; architecture regressions; CI/test configuration; evidence.
Architecture Constraints: Registry catalog authority; Runtime executor; Record immutable; PostgreSQL durable; no concrete transforms.
Out of Scope: new Transform primitives, marketplace, SDK, sandbox, economic/data-plane redesign.
Acceptance Criteria: `W013-AC01` through `W013-AC11`.
Required Verification: PG end-to-end; tenant/isolation/idempotency/failure tests; anti-dependency; all gates; Architect Review.
Definition of Done: objectively verified; merged; VERIFIED.
...
Repository Scope: `spec/` architecture/change-control documents, tests, dependency graph, records, evidence.
Architecture Constraints: V3 frozen during review; no production implementation; no Prisma; no sandbox selection.
Out of Scope: Extension production code, sandbox, marketplace/SDK, cryptographic mechanism, schema redesign, DOM-P05..P08 promotion.
Acceptance Criteria: `W014-AC01` through `W014-AC11`.
Required Verification: ACR completeness; candidate V4 consistency; security/lifecycle invariants; anti-dependencies; V3 immutability; DOM-P04 non-promotion; gates; Architect Review.
...
Repository Scope: governance/specification, Work Item/dependency records, regression tests, evidence.
...
Required Verification: ACR/V4 state; V3 immutability; freeze/promotion tests; all gates; Architect Review.
...
Out of Scope: Registry redesign except compliance fixes; durable provenance; sandbox selection/implementation; concrete extensions; Marketplace/SDK; economics; V4 changes.
...
Architecture Constraints: V4 immutable; ACR only; no sandbox implementation; no V5 freeze in this Work Item.
...
Architecture Constraints: preserve V4 immutable; freeze only the approved WASI Component Model/capability-sandbox contract; do not freeze a concrete WASI revision/runtime; keep resource quantities distinct; retain deny-by-default fallback; no production implementation.
