...
export async function executeDispatchAssignment(
  ...
  try {
    ...
    // --- Baseline calculation (VPP-2: real baseline engine) ---
    // Previously: baseline = 0 (placeholder, acknowledged as simulation fixture).
    // Now: uses the SimilarDayAverageBaseline strategy, which is the most
    // practical baseline for VPP dispatch (accounts for weekday/weekend patterns).
    //
    // In production, historical telemetry would be loaded from the asset's
    // event history. For the MVP, we use a simulated history generated by
    // the DERHistorySimulator with known ground truth.
    const actualKwh = new Prisma.Decimal(dischargeResult.actualKwh)

    // Generate a synthetic history for baseline prediction.
    // In production, this would query historical events from the database.
    const { DERHistorySimulator } = await import('./der-simulator.service')
    const { SimilarDayAverageBaseline } = await import('./baseline-engine.service')
    const simulator = new DERHistorySimulator(42)
    const syntheticHistory = simulator.generateHistory(
      14, // 14 days of history
      Math.floor(assignment.dispatch.startTime.getHours()),
      Math.ceil((assignment.dispatch.endTime.getTime() - assignment.dispatch.startTime.getTime()) / 3600000),
      parseFloat(assignment.assignedKw),
    )
    const baselineStrategy = new SimilarDayAverageBaseline()
    const baselinePrediction = baselineStrategy.predict(syntheticHistory.days, syntheticHistory.dispatchDay)
    ...
    const baseline = await db.vppBaseline.create({
      data: {
        ...
        metadataJson: JSON.stringify({
          ...
          trueCounterfactualKwh: syntheticHistory.dispatchDay.trueCounterfactualKwh,
          trueIncrementalKwh: syntheticHistory.dispatchDay.trueIncrementalKwh,
        }),
      },
    })
