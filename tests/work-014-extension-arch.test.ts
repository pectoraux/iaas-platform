...
describe('WORK-014/015 — ACR-003 and V4 freeze', () => {
  ...
  test('V4 is the current canonical frozen architecture', () => {
  ...
  test('Extension contract is frozen without concrete implementation', () => {
    expect(arch).toContain('Extension')
    expect(arch).toContain('extensionType + extensionVersion')
    expect(arch).toContain('FROZEN-CONTRACT')
    expect(arch).toContain('Concrete extensions are future')
  })

  test('Registry/runtime responsibility split remains explicit', () => {
  ...
  test('V4 dependency graph is frozen and acyclic', () => {
    ...
    expect(graph).toContain('Extension → Transform')
