...
Task ID: 12B-slice-1
Agent: main (Z.ai Code)
Task: Phase 12B slice 1 — implement the generic control-plane contracts + deterministic scheduler above the frozen kernel. This is the first implementation slice per the frozen Phase 12A spec (ea83522). The user's GitHub integration returned 403 on writes, but my git push token works.

Work Log:
- Implemented src/lib/control-plane/types.ts: the generic control-plane contracts per the frozen spec. Includes: ParticipantIdentity, ParticipantMembership, ParticipantRole (roles separated from membership); ResourceIdentity (no networkId), NetworkResourceMembership (binds to ParticipantMembershipId), ResourceKind discriminator; NetworkRequest (actor-neutral, authorized by requesterMembershipId); CommitmentConstraint base + CapacityConstraint (depletes capacity) vs ServiceConstraint (SLA, no depletion); AllocationDecision (with decisionSnapshotHash + schedulerVersion); Network Scope Integrity helpers (verifyNetworkScopeIntegrity, assertNetworkScopeIntegrity, NetworkScopeIntegrityError); authorizeRequest (request isolation); computeDecisionSnapshotHash (§8.7 reproducibility); createNetworkRequest factory.
- Implemented src/lib/control-plane/scheduler.ts: the deterministic, vertical-neutral scheduler. Algorithm: (1) authorize the request (request isolation — §6.6), (2) verify Network Scope Integrity (§8.6), (3) filter candidates (active, offer requested capabilities, have enough remaining capacity, available within time window, same network), (4) sort deterministically by membershipId, (5) select first eligible, (6) compute decisionSnapshotHash, (7) return AllocationDecision. PURE: no side effects, no DB, no kernel imports. SCHEDULER_VERSION = 'deterministic-v1'.
- Implemented src/lib/control-plane/index.ts: public exports, no side effects on import.
- Wrote tests/phase-12b-control-plane.test.ts: 20 tests covering: Network Scope Integrity (§8.6 — same/cross network, assert throws, scheduler rejects cross-network); Request isolation (§6.6 — active consumer accepted, suspended rejected, no consumer/orchestrator role rejected, orchestrator accepted, scheduler rejects suspended); Scheduler reproducibility (§8.7 — same inputs → same decision, different capacity → different hash, deterministic candidate selection by membershipId); Capacity correctness (insufficient capacity rejected, missing capability rejected); Constraint distinction (§6.7 — CapacityConstraint has capacitySourceId, ServiceConstraint does not); Vertical-neutrality (no vertical imports, no kernel imports, no hard-coded vertical capability types).
- Fixed a typecheck issue: CapacityRequirement → CapabilityRequirement (the spec uses CapabilityRequirement for request requirements, CapacityEntry for resource capacity).
- VERIFICATION: tsc zero errors in new files. eslint clean. 20/20 phase-12b tests pass. 108/108 tests pass across phase-12b + phase-11b + phase-10 + architecture. Dev server HTTP 200.
- HONEST STATUS: This is slice 1 only. The control-plane contracts + scheduler are implemented and tested (in-memory, pure). The persistence layer (DB models for ResourceIdentity, ParticipantMembership, NetworkRequest, AllocationDecision), the control-plane API endpoints, the NetworkLaunch atomic operation, and the integration with the kernel's NetworkRuntime are NOT yet implemented — those are subsequent slices. The scheduler is pure and vertical-neutral; it does not bypass the kernel.

Stage Summary:
- Phase 12B slice 1 delivered: src/lib/control-plane/{types,scheduler,index}.ts + tests/phase-12b-control-plane.test.ts.
